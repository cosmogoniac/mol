...
function menu(){
	...
	//videos
		if(document.querySelector(".vidone"))
			videos();
	//videos post
		if(document.querySelector(".vidpost"))
			vidpost();
	//imagenes
		if(document.querySelector("p .centrar"))
			centrar();
}

function centrar(){
	var centrar=document.querySelectorAll(".centrar");
	for(i in centrar)
		if(centrar[i].tagName)
			centrar[i].parentNode.className="aucentre";
}

function vidpost(){
	var iframe=document.querySelector(".vidpost");
	var src=document.querySelector(".ytbon");
	// alert(src.value);
